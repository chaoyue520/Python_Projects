...
try:
    if int(time.strftime("%w")) == 1:
        send_hi(str('./daily_data_')+\
        	datetime.datetime.strftime(datetime.datetime.now() - datetime.timedelta(days=3),'%Y_%m_%d')+\
        	str('.txt'))
        sendhi('近三天报损，请相关负责人查看')
        print(str('星期')+str(int(time.strftime("%w")))+str(':task success!'))
    elif int(time.strftime("%w")) in (2,3,4,5):
        send_hi(str('./daily_data_')+\
        	datetime.datetime.strftime(datetime.datetime.now() - datetime.timedelta(days=1),'%Y_%m_%d')+\
        	str('.txt'))
