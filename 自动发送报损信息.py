...
def send_uid_list(filename):
    content=pd.read_table(filename
              ...
              ,sep='\t')
    #分别求出笔数，人数，和金额    
    send_uid_list=content[['grouptype','user_id']].drop_duplicates()
    send_uid_list.to_csv('b.txt',index=None,sep='\t')
    with open('b.txt','r') as file:
        content=file.read()
    # 设置发送格式
    data = {}
    data['msg'] = content
    data['to_group'] = '1587585'
...
try:
    if int(time.strftime("%w")) == 1:
        filename=str('/home/sas/jiwenchao/ods_mg_case_1112/daily_data_list/daily_data_')+datetime.datetime.strftime(datetime.datetime.now() - datetime.timedelta(days=3),'%Y_%m_%d')+str('.txt')
        ...
        if int(a) == 0 : 
            ...
            send_hi(filename)
			# 发送报损uid列表
            send_uid_list(filename)
            sendhi('近三天报损汇总及报损uid，请相关负责人查看')
            print(str('星期')+str(int(time.strftime("%w")))+str(':task success!'))
    elif int(time.strftime("%w")) in (2,3,4,5):
        filename=str('/home/sas/jiwenchao/ods_mg_case_1112/daily_data_list/daily_data_')+datetime.datetime.strftime(datetime.datetime.now() - datetime.timedelta(days=1),'%Y_%m_%d')+str('.txt')
        a=os.popen(str('wc -l ')+filename).readlines()[0].split( )[0]
        if int(a) == 0 : 
            sendhi('昨日无报损')
        else : 
